...
public final class WhileStmt extends Stmt {
    ...
    public WhileStmt(@NotNull LoopPair loopPair, @NotNull Stmt stmt) {
    ...
    public @NotNull UnitValue eval(@NotNull ValEnv env) throws RuntimeError {
        IntegerValue upperBound = (IntegerValue) loopPair.eval(env);
        Symbol s = loopPair.getSymbol();
        ValEnv loopEnv = ValEnv.fromOuter(env);
        loopEnv.put(s, IntegerValue.ZERO);
        while (true) {
            IntegerValue i = (IntegerValue) loopEnv.get(s);
